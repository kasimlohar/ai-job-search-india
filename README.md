...
```powershell
$tools = @("jobbank-search", "jobdanmark-search", "jobindex-search", "jobnet-search", "linkedin-search", "freehire-search", "remoteok-search", "weworkremotely-search", "himalayas-search")
...
```bash
for tool in jobbank-search jobdanmark-search jobindex-search jobnet-search linkedin-search freehire-search remoteok-search weworkremotely-search himalayas-search; do
  (cd .agents/skills/$tool/cli && bun install)
done
```

For `linkedin-search`, `freehire-search`, `remoteok-search`, `weworkremotely-search`, and `himalayas-search` the install is optional: they have zero runtime dependencies and run with plain `bun`; `bun install` only pulls TypeScript dev types.
...
│   ├── remoteok-search/               # RemoteOK (remote-only listings, worldwide)
│   ├── weworkremotely-search/         # We Work Remotely (remote-only listings, worldwide)
│   └── himalayas-search/              # Himalayas (remote-only listings, worldwide)
├── cv/
