...
1. Check that Notion MCP tools are available in this session (tool names starting with `mcp__notion__` or similar). Determine this from the session's own tool list **only** - never by running shell commands to list servers, which would interrupt the user with a permission prompt before the graceful exit. If the tools are not available, stop and tell the user how to connect:
   > Notion MCP isn't connected. Enable it for this run via the `notion` entry in `opencode.json` (`mcp.notion.enabled`, remote `https://mcp.notion.com/mcp`), then start a **new session** (servers added mid-session are only picked up on restart), complete the OAuth login there, and re-run `/notion-sync`.
   > (Claude Code equivalent: `claude mcp add --transport http notion https://mcp.notion.com/mcp`, then `/mcp` to log in.)
